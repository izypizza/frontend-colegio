...
export default function BibliotecariosPage() {
  const { user } = useAuth();
  const permissions = usePermissions("bibliotecarios");
  ...
  const fetchData = async () => {
    try {
      ...
      handleError(err, "Error al cargar bibliotecarios");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setSuccess("Bibliotecario creado");
      closeCreate();
      setCreateForm({ name: "", email: "", password: "", is_active: true });
      await fetchData();
    } catch (err: any) {
      handleError(err, "Error al crear bibliotecario");
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setSuccess("Bibliotecario actualizado");
      closeEdit();
      setEditingUser(null);
      await fetchData();
    } catch (err: any) {
      handleError(err, "Error al actualizar bibliotecario");
    }
  };
    ...
    <PermissionGuard
      ...
      fallback={
        <div className="p-6">
          <Alert
            type="error"
            message="Solo administradores pueden gestionar bibliotecarios."
          />
        </div>
      }
    >
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Bibliotecarios</h1>
            <p className="text-gray-600">Gestiona cuentas de bibliotecarios</p>
          </div>
          {permissions.canCreate && (
            <Button onClick={openCreate}>Nuevo bibliotecario</Button>
          )}
        </div>
        ...
        >
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="create_active" className="text-sm text-gray-700">
                Usuario activo
              </label>
            </div>
