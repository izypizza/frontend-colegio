...
export default function MisAsistenciasPage() {
  ...
  const prepararDatosGraficas = () => {
    ...
    const distribucion = {
      ...
      tardanza: data.asistencias.filter(a => a.estado === 'tardanza').length,
      justificado: data.asistencias.filter(a => a.estado === 'justificado').length
    };

    const pieData = [
      { name: 'Presente', value: distribucion.presente, color: '#10B981' },
      { name: 'Ausente', value: distribucion.ausente, color: '#EF4444' },
      { name: 'Tardanza', value: distribucion.tardanza, color: '#F59E0B' },
      { name: 'Justificado', value: distribucion.justificado, color: '#3B82F6' }
    ...
    data.asistencias.forEach(a => {
      ...
      if (a.estado === 'tardanza') asistenciasPorMateria[materiaId].tardanza++;
    });
        ...
        <Card>
          <div className="flex items-center justify-between">
            ...
            <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
              (data?.estadisticas.porcentaje_asistencia || 0) >= 80 ? 'bg-green-100' : 'bg-red-100'
            }`}>
      ...
      {data?.asistencias && data.asistencias.length > 0 && graficasData && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">📊 Distribución de Asistencias</h3>
            ...
            <Card className="lg:col-span-2">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">📚 Asistencia por Materia</h3>
              <ResponsiveContainer width="100%" height={250}>
