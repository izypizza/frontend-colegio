...
export default function EstudiantesPage() {
  ...
  const fetchData = async (options?: { page?: number; perPage?: number }) => {
    try {
      setLoading(true);
      const startTime = performance.now();

      const pageToFetch = options?.page ?? pagination.currentPage;
      const perPageToFetch = options?.perPage ?? pagination.perPage;
      ...
      if (
        ...
        pagination.updatePagination({
          ...
          perPage: (estudiantesData.per_page as number) || perPageToFetch,
        });
      } else {
